...
PATH_LFW = r"Modules\Facial Recognition\lfw-deepfunneled\lfw-deepfunneled"

# --- NUEVAS RUTAS DE GUARDADO ---
...
    # ... (Todo el código de ploteo y métricas, sin cambios en la lógica)
    ...
    # Llamar a la función de ploteo extra (para la nueva curva)
    # Requerirá que la definas antes de correr el script principal
    # Pero aquí la estamos llamando como si existiera para la lógica
...
# ===================================================================
#           BLOQUE PRINCIPAL MODIFICADO (CÓMO GUARDAR)
# ===================================================================
...
if __name__ == "__main__":
    ...
    if os.path.exists(SCORES_FILE) and os.path.exists(LABELS_FILE):
        ...
        # Para que la función plot_full_report no falle, necesita una lista subject_metrics_list
        # La creamos vacía o simulada, ya que no se usa para las métricas globales
        subject_metrics_list = [{'name': 'Cargado', 'f1': 0.99}] 
    
    # 3. EJECUCIÓN DEL BENCHMARK (Si los archivos NO existen)
    else:
        print("\n--- ARCHIVOS NO ENCONTRADOS. Iniciando Benchmark Completo ---")
        
        # 1. BUSCAR SUJETOS VÁLIDOS (Lógica anterior)
        ...
        # 2. INDEXAR POOL DE IMPOSTORES (Lógica anterior)
        ...
        # 3. EJECUCIÓN DEL BENCHMARK (Lógica anterior)
        ...
        for subject in tqdm(valid_subjects, desc="Procesando Sujetos"):
            ...
            # A. FASE DE ENROLLMENT (Lógica anterior)
            ref_imgs = photos[:5]
            test_imgs = photos[5:]
            ...
            # B. FASE DE TEST GENUINO (Lógica anterior)
            for p in test_imgs:
                emb = get_embedding(p)
                if emb is not None:
            ...
            # C. FASE DE TEST IMPOSTOR (Lógica anterior)
            ...
            # Calcular métrica rápida para este sujeto (Lógica anterior)
            if local_y_true:
        ...
        # 4. GUARDAR LOS DATOS PESADOS UNA VEZ FINALIZADO EL BENCHMARK
    ...
    if global_y_true:
        # Volver a llamar a la función de ploteo para que se ejecute con los arrays correctos
        # La función de ploteo extra (FAR/FRR) se llama DENTRO de plot_full_report
